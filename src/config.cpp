...
static const std::unordered_map<AStringView, AStringView> CONFIG_COMMENTS = {
      ...
      "Should match with papik_chat_id account name." },
    {
      ...
      "Felicity's kernel gives several benefits to the \"instance owner user\":\n"
      "- Felicity can't ban instance owner\n"
      "- Messages from instance owner are prioritized (processed ASAP)\n"
      "- A message from instance owner wakes up Felicity if she sleeps\n"
      "\n"
...
#undef X
    ...
    toml.at("general").comments() = std::vector<std::string> {
        R"(# Felicity AI Kernel project
# WARNING - SENSITIVE DATA! DO NOT SHARE NOR COMMIT THIS FILE!!!
#
# This file contains all the settings available in Felicity's kernel.
# To adjust prompts, go to prompts/ directory.
# If Felicity is running, it will autoreload changes to this file and prompts automatically.
...
#
# В этом файле содержатся все настройки, которые доступны в ядре Фелисити.
# Если хочешь поменять промпты, сходи в папку prompts/.
# Если Felicity уже запущено, оно само перезагрузит изменения в этот файл и промпты.
